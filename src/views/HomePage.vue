<template>
  <div id="homepage" class="container-fulid">
    ...
    <div id="homepage-bar" class="sticky-top text-nowrap">
      <nav class="navbar navbar-expand-sm bg-white navbar-light shadow">
        <span class="navbar-text text-primary mx-5 h4">
          <img src="@/assets/icon.png" style="width: 33px;" class="align-bottom">
          夏至的主页
        </span>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#homepage-bar-items">
          <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="homepage-bar-items">
          ...
          <ul class="nav nav-pills container-fulid justify-content-end ps-3 me-5">
            <router-link to="/login?type=login">
              <li v-show="!isLogin"><button class="btn btn-primary">登录</button></li>
            </router-link>
            <router-link to="/login?type=logup">
              <li v-show="!isLogin"><button class="btn">注册</button></li>
            </router-link>
            <li v-show="isLogin" id="homepage-self-icon">
              ...
              <transition name="showSelfIcon" tag="show-self">
                <div v-show="showSelfCard" id="homepage-self-icon-card" ref="selfIconInfo" class="card"
                  style="width: 150px">
                  <div class="card-body">
                    <h4 class="card-title">夏至</h4>
                    <p class="card-text text-wrap">说点什么吧说点什么吧说点什么吧</p>
                    <button class="btn btn-sm btn-primary float-start">个人</button>
                    <button class="btn btn-sm btn-danger float-end">退出</button>
                  </div>
                </div>
              </transition>
            </li>
            <li v-show="isAdmin & isLogin" class="nav-item ms-3">
              <router-link to="setting">
                <a class="nav-link text-dark" :class="[barSelected == 5 ? 'active text-white' : '']"
                  @click="barClick(5)">设置</a>
              </router-link>
            </li>
          </ul>
        </div>
    ...
    <div id="homepage-content" class="container d-flex flex-wrap justify-content-around">
      ...
      <div v-for="i in amount" :key="i" class="card m-5" style="width:200px">
        ...
        <div class="card-body">
          <h4 class="card-title">夏至</h4>
          <p class="card-text">夏至的个人网站夏至的个人网站夏至的个人网站</p>
        </div>
      </div>
    </div>
  </div>
